...
### A. IA Relationnelle (Messagerie)
*   **Objectif** : Générer des messages directs (Email/WhatsApp) de conversion.
*   **Input Context** :
...
*   **Stockage des résultats** : Table `dash_marketing_messages`.
*   **Source de données** : `messaging.ts` -> `generateMessagingSuggestionsAction`.
...
1. L'action `saveMessagingFeedbackAction` est appelée.
2. La table `dash_marketing_messages` est mise à jour (colonnes `is_liked`, `admin_feedback`).
