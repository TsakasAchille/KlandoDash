...
export function TripMapPopup({ trip, onClose }: TripMapPopupProps) {
  return (
    <Card className="absolute top-4 right-4 w-80 z-[1000] bg-[#081C36] border-gray-700 shadow-xl">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg text-[#EBC33F]">
            Détails du trajet
          </CardTitle>
          <Button
            variant="ghost"
      ...
      <CardContent className="space-y-4 text-white">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-300">{formatDate(trip.departure_schedule || "")}</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        {/* Prix + Status */}
        <div className="flex items-center justify-between">
          <span className="text-lg font-bold text-[#EBC33F]">
            ...
            <div className="flex flex-wrap gap-2">
              {trip.passengers.map((p) => (
                <Link key={p.uid} href={`/users?selected=${p.uid}`}>
                  <div className="w-6 h-6 rounded-full bg-gray-700 flex items-center justify-center overflow-hidden cursor-pointer hover:ring-2 ring-[#EBC33F] transition-all">
                    {p.photo_url ? (
                      <img
                        src={p.photo_url}
                        alt={p.display_name || ""}
        ...
        <div className="border-t border-gray-700 pt-3">
          <Link href={`/trips?selected=${trip.trip_id}`}>
            <Button className="w-full bg-[#7B1F2F] hover:bg-[#7B1F2F]/80 text-white">
