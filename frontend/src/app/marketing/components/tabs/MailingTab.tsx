...
export function MailingTab({ 
  ...
  return (
    <div className="flex gap-6 h-[750px] animate-in fade-in duration-500">
            ...
            filteredEmails.map((email) => (
              <div 
                ...
                className={cn(
                  ...
                  selectedEmailId === email.id ? "bg-purple-50/50 border-l-4 border-l-purple-600" : ""
                )}
              >
      ...
      {/* 3. EDITOR / VIEWER */}
      {selectedEmail && (
        <div className="flex-1 bg-slate-50 border border-slate-200 rounded-[2rem] overflow-hidden flex flex-col animate-in slide-in-from-right-4 duration-500 shadow-2xl">
          {/* Header Editor */}
          <div className="p-6 border-b border-slate-200 bg-white flex items-center justify-between">
            <div className="flex items-center gap-4 text-left">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] font-bold text-slate-600 uppercase mt-0.5">À: {selectedEmail.recipient_email}</p>
              </div>
            </div>
            <button onClick={() => setSelectedEmailId(null)} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-500 shrink-0">
            ...
            {isEditing ? (
                <div className="h-full flex flex-col gap-4 text-left">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-[9px] text-slate-500 font-medium italic text-left">Note: HTML supporté pour les images.</p>
                    </div>
                    <Textarea 
        ...
        <DialogContent className="sm:max-w-[600px] bg-slate-50 border-slate-300 rounded-[2.5rem] p-0 overflow-hidden outline-none shadow-2xl">
            <div className="flex flex-col h-[600px]">
                <DialogHeader className="p-8 pb-6 bg-white border-b border-slate-200 text-left">
                    ...
                    <DialogDescription className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mt-1 text-left">Rédiger un mail de croissance</DialogDescription>
                </DialogHeader>
                ...
                <DialogFooter className="p-6 bg-slate-50 border-t border-slate-200 gap-3">
                    <Button variant="ghost" onClick={() => setIsComposeOpen(false)} className="rounded-xl text-slate-600 font-black text-[10px] uppercase hover:bg-slate-200 transition-colors">Annuler</Button>
                    <Button 
