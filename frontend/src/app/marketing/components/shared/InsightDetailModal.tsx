            ...
            <DialogHeader className="p-10 pb-8 border-b border-white/5 relative z-10 bg-white/[0.02]">
              <div className="flex items-center justify-between mb-6 text-left">
                  ...
                  <div className="flex items-center gap-6 text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">
                      <div className="flex items-center gap-2"><Calendar className="w-3.5 h-3.5 text-klando-gold" /> {new Date(insight.created_at).toLocaleDateString('fr-FR')}</div>
                      <div className="flex items-center gap-2"><Clock className="w-3.5 h-3.5 text-klando-gold" /> {new Date(insight.created_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</div>
                  </div>
              </div>
