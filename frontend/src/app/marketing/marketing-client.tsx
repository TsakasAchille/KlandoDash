...
import { 
  ...
  FileText, ArrowRightCircle, Calendar, Clock, Mail, Send
} from "lucide-react";
...
export function MarketingClient({ 
  ...
  const handleMailingScan = async () => {
    ...
    try {
      ...
      toast.error("Erreur lors de l'analyse mailing.");
    } finally {
      ...
      <Tabs value={tabParam} onValueChange={handleTabChange} className="space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card/30 p-2 rounded-3xl border border-white/5 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            {tabParam === "mailing" && (
                <Button 
                onClick={handleMailingScan}
                disabled={isScanningMailing}
                size="sm"
                className="bg-purple-600 hover:bg-purple-700 text-white font-black rounded-2xl px-6 h-10 shadow-lg shadow-purple-500/20"
                >
        ...
        <TabsContent value="mailing" className="outline-none space-y-8">
          {/* Drafts Section */}
