"use client";

import { cn } from "@/lib/utils";
import { Search, Filter, Mail, CheckCircle2, Inbox, MessageSquare } from "lucide-react";
import { MarketingMessage } from "@/app/marketing/types";

interface MessageListProps {
  messages: MarketingMessage[];
  selectedId: string | null;
  onSelect: (id: string) => void;
  activeFolder: string;
}

export function MessageList({ messages, selectedId, onSelect, activeFolder }: MessageListProps) {
  return (
    <div className="flex-1 bg-white border border-slate-200 rounded-2xl lg:rounded-[2rem] overflow-hidden shadow-xl flex flex-col transition-all duration-500 h-full min-h-0">
      <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between text-left shrink-0">
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 pl-2">{activeFolder}</h3>
        <div className="flex items-center gap-2 pr-2">
          <Search className="w-3.5 h-3.5 text-slate-400" />
          <Filter className="w-3.5 h-3.5 text-slate-400" />
        </div>
      </div>

      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {messages.length > 0 ? (
          messages.map((msg) => (
            <div 
              key={msg.id}
              onClick={() => onSelect(msg.id)}
              className={cn(
                "flex items-center gap-3 lg:gap-4 px-4 lg:px-6 py-3 lg:py-4 border-b border-slate-50 cursor-pointer transition-all hover:bg-slate-50 group text-left",
                selectedId === msg.id ? "bg-purple-50/50 border-l-4 border-l-purple-600" : ""
              )}
            >
              <div className="flex-shrink-0">
                {msg.status === 'SENT' ? (
                  <CheckCircle2 className="w-4 h-4 text-green-600" />
                ) : msg.channel === 'WHATSAPP' ? (
                  <MessageSquare className="w-4 h-4 text-[#25D366] group-hover:scale-110 transition-transform" />
                ) : (
                  <Mail className="w-4 h-4 text-slate-400 group-hover:text-purple-600" />
                )}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-0.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-xs font-black text-slate-900 uppercase truncate">
                      {msg.recipient_name || msg.recipient_contact.split('@')[0]}
                    </span>
                    <span className={cn(
                      "text-[7px] font-black px-1 rounded uppercase",
                      msg.channel === 'WHATSAPP' ? "bg-green-50 text-green-600" : "bg-blue-50 text-blue-600"
                    )}>
                      {msg.channel}
                    </span>
                  </div>
                  <span className="text-[10px] font-bold text-slate-400 tabular-nums whitespace-nowrap ml-2">
                    {new Date(msg.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}
                  </span>
                </div>
                <p className="text-[11px] font-bold text-slate-700 truncate">{msg.subject || "(Pas d'objet)"}</p>
                <p className="text-[10px] text-slate-500 truncate mt-0.5">{msg.content.substring(0, 60)}...</p>
              </div>
            </div>
          ))
        ) : (
          <div className="h-full flex flex-col items-center justify-center opacity-40 italic text-slate-400">
            <Inbox className="w-12 h-12 mb-4" />
            <p className="text-[10px] font-black uppercase tracking-widest text-center">Dossier vide</p>
          </div>
        )}
      </div>
    </div>
  );
}
