-- Migration 073: Update RPC stats to handle last_status for ARCHIVED trips
-- Description: Use last_status as effective status when status is ARCHIVED and include paid_count.
...
DECLARE
    ...
    txns_trends json;
BEGIN
    -- SECTION: TRANSACTIONS & REVENUE (Besoin de rev_count pour les trajets payés)
    SELECT count(*) INTO txns_total_val FROM transactions;
    SELECT coalesce(sum(amount), 0) INTO txns_amount FROM transactions;
    
    SELECT coalesce(sum(t.amount), 0), coalesce(sum(tr.driver_price), 0), count(DISTINCT tr.trip_id) 
    ...
    -- Utilisation du statut effectif (last_status si ARCHIVED)
    SELECT json_agg(t) INTO trips_by_status FROM (
        SELECT 
            CASE 
                WHEN status = 'ARCHIVED' AND last_status IS NOT NULL THEN last_status 
                ELSE status 
            END as status, 
            count(*) as count 
        FROM trips 
    ...
    -- Calcul du taux d'annulation et sièges moyens (sur le statut effectif)
    SELECT 
        round((count(*) FILTER (WHERE (CASE WHEN status = 'ARCHIVED' AND last_status IS NOT NULL THEN last_status ELSE status END) = 'CANCELLED')::numeric / NULLIF(count(*), 0)::numeric) * 100, 2),
    ...
    -- Top drivers basés sur COMPLETED ou CLOSED
    SELECT json_agg(t) INTO top_drivers_list FROM (
        ...
        WHERE (CASE WHEN t.status = 'ARCHIVED' AND t.last_status IS NOT NULL THEN t.last_status ELSE t.status END) IN ('COMPLETED', 'CLOSED')
        GROUP BY u.uid, u.display_name, u.photo_url
        ORDER BY trips_count DESC
        LIMIT 5
    ) t;
    
    -- SECTION: USER TYPOLOGY (Based on Actions)
    ...
    -- SECTION: REGISTRATION TYPOLOGY (Based on Role field)
    SELECT json_build_object(
        'drivers', (SELECT count(*) FROM users WHERE role = 'driver'),
        'passengers', (SELECT count(*) FROM users WHERE role = 'passenger' OR role IS NULL)
    ) INTO reg_typology;

    -- TRENDS (Weekly)
    SELECT json_agg(t) INTO txns_trends FROM (
        SELECT 
    ...
    SELECT json_agg(t) INTO orphan_cities FROM (
        ...
        WHERE NOT EXISTS (
            ...
            AND (CASE WHEN tr.status = 'ARCHIVED' AND tr.last_status IS NOT NULL THEN tr.last_status ELSE tr.status END) = 'ACTIVE'
        )
        GROUP BY city
    ...
    RETURN json_build_object(
        'trips', json_build_object(
            ...
            'paid_count', coalesce(rev_count, 0),
        ...
        'users', json_build_object(
            ...
            'registrationTypology', coalesce(reg_typology, '{"drivers": 0, "passengers": 0}'::json)
        ),
        ...
        'transactions', json_build_object(
            ...
            'trends', coalesce(txns_trends, '[]'::json)
        ),
